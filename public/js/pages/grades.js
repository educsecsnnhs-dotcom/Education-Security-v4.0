...
(async function () {
  ...
  async function loadViews() {
    ...
    if (user.role === "Student") {
      ...
      try {
        const sectionsRes = await PageUtils.fetchJson("/api/sections", { headers: authHeaders() });
        if (sectionsRes.ok) {
  ...
  async function loadGrades() {
    ...
    try {
      let res = null;
      if (v === "me") {
        res = await PageUtils.fetchJson("/api/grades/me", { headers: authHeaders() });
      } else if (v && v.startsWith("section:")) {
        const sec = v.split(":", 2)[1];
        res = await PageUtils.fetchJson(`/api/grades?section=${sec}`, { headers: authHeaders() });
      } else if (v && v.startsWith("subject:")) {
        const sub = v.split(":", 2)[1];
        res = await PageUtils.fetchJson(`/api/grades?subject=${sub}`, { headers: authHeaders() });
      } else if (v === "all") {
        res = await PageUtils.fetchJson("/api/grades", { headers: authHeaders() });
      }
      ...
      items.forEach((g) => {
        ...
        const student =
          g.studentName || (g.student && g.student.name) || g.name || "Student";
        const subject =
          g.subjectName || (g.subject && g.subject.name) || g.subject || "";
        const grade = g.grade || g.score || "";

        tr.innerHTML = `
          <td style="padding:8px;border-top:1px solid #eef2ff">${PageUtils.escapeHtml(student)}</td>
          ...
          <td style="padding:8px;border-top:1px solid #eef2ff">
            <input data-id="${g._id || g.id}" value="${PageUtils.escapeHtml(String(grade))}" class="input" style="width:80px"/>
          </td>
          <td style="padding:8px;border-top:1px solid #eef2ff">
